...
public Object getValueAt(int row, int col) {
    // Memeriksa apakah data memiliki elemen yang cukup
    if (row >= 0 && row < data.size()) {
        // Menyimpan elemen dari List data dari baris yang dipilih ke Biodata rowItem
        Biodata rowItem = data.get(row);
        
        // Membuat variabel value untuk menyimpan nilai dari kolom yang dipilih
        String value = "";

        // Switch case untuk menentukan nilai dari variabel value
        switch (col) {
            // Jika col bernilai 0
            case 0:
            ...
            case 3:
                ...
                break;
            // Kasus-kasus lain di sini ...
        }

        return value;
    } else {
        // Jika row tidak valid, kembalikan nilai default atau handle dengan cara lain
        return null; // atau throw exception, tergantung pada kebutuhan aplikasi Anda
    }
}
